...
class _VideoPhotoPageState extends State<VideoPhotoPage> {
  late final HouseProcessMediaBloc bloc;
  @override
  void initState() {
    super.initState();
    bloc = context.read<HouseProcessMediaBloc>();
  }
  ...
  Widget build(BuildContext context) {
    ...
    return BlocListener<HouseProcessMediaBloc, HouseProcessMediaState>(
      listener: (context, state) {
        state.status.maybeWhen(
          loading: () {
            context.appDialog.showLoading(isDismiss: true);
          },
          orElse: () {
                  ...
                  if (media.length >= 3)
                    Expanded(
                      child: SizedBox(
                        ...
                        child: Stack(
                          children: [
                            Positioned.fill(child: image(media[2])),
                            if (media.length > 3)
                              Positioned.fill(
                                child: Container(
                    ...
                    .joinWidget(AppSize.mediumWidthDimens.horizontalSpace)
                    .toList(),
              );
            },
          ),
